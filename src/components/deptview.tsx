...
import Link from 'next/link';
import DepartureForm from './departuremodal';
...
const Departurelist: React.FC = () => {
  ...
  const [data, setData] = useState<Array<Departure>>([]);
  ...
  const handleCreate = async (departureData: { departure: string }) => {
  ...
  const handleEdit = async (departureData: { departure: string }) => {
    if (!selected) return;
    ...
    if (res.ok) {
      ...
      setData(data.map((dept) => (dept.id === selected.id ? updatedDept : dept)));
      setMode(null);
  ...
  const openEditForm = (departure: Departure) => {
    setMode('edit');
  ...
  return (
    ...
    <div className="min-h-screen">
      ...
      onClick={openCreateForm}>Create Departure</button></div>

      {mode === 'create' && <DepartureForm onSubmit={handleCreate} onClose={() => setMode(null)} />}
      {mode === 'edit' && selected && (
        <DepartureForm
...
<Table striped bordered hover>
        <thead>
          <tr className="bg-gray-100 border-b">
            ...
            <th className="py-2 px-4 text-left w-12/12">Departure</th>
        ...
        <tbody>
          {data.map((dep, index) => (
            <tr key={dep.id} className="border-b">
              ...
              <td className="py-2 px-4">{dep.departure}</td>
