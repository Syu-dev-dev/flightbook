...
interface FlightnoFormProps {
  onSubmit: (data: { flightno: string}) => void;
  flightnodata?: { id: number, flightno: string };
  onClose: () => void;
}

const FlightnoForm: React.FC<FlightnoFormProps> = ({ onSubmit, flightnodata, onClose }) => {
  const [flightno, setName] = useState(flightnodata?.flightno || '');
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-60">
      <div className="bg-white p-10 rounded-xl shadow-xl w-full max-w-4xl transform transition-transform duration-300 scale-100">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-gray-900">{flightnodata ? 'Edit Airline' : 'Create FlightNO.'}</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            <div>
              <label htmlFor="name" className="block text-lg font-medium text-gray-700">Flight Number</label>
              <input
                type="text"
                id="name"
                value={flightno}
